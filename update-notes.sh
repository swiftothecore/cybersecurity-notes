#!/bin/bash
# Swap a newly downloaded cybersecurity-notes-vNN.html into the tracked file,
# commit it, and push. Run from anywhere.
set -e
REPO="/Users/corey/Documents/Cybersecurity Notes"

# Use the file passed as $1, or auto-detect the newest cybersecurity-notes-v*.html
# dropped straight into the repo folder (excludes the tracked index.html itself)
if [ -n "$1" ]; then
  SRC="$1"
else
  SRC=$(ls -t "$REPO"/cybersecurity-notes-v*.html 2>/dev/null | head -1)
fi
